...
- При запуске команд в консоли не использует: `2>&1` (приводит к зависанию команды)
- Используй команды из `Makefile` для запуска: проекта, контейнеров докера, команд artisan, phpunit тестов, консольных команд
- Используй переменные из.env файла

## Локализация
- Добавлять краткое описание в phpdoc для всех методов и классов на русском языке, после описания добавлять пустую строку и далее описание аргументов.
...
- Длина строк кода не должна превышать `120 символов` в одной строке, разбивай логически даже если ломает стиль, используй `php-cs-fixer` с правилом `line_length=120`. Не допускать длинных цепочек вызовов или массивов в одну строку, если они выходят за лимит.
Пример:
```php
# допустимо
$user = User::query()
...
# недопустимо (слишком длинно)
$user = User::query()->where('email', $email)->where('status', 'active')->->where('type', 'default')->withCache()->first();
```
...
- Вставляй пустую строку:
  - перед оператором `default` в `match` и `switch`;
...
## Стиль PHPDoc для классов app/src
- Перед классом добавляй PHPDoc с кратким описанием общей сути.
- Для каждого метода (кроме __construct) добавляй PHPDoc следующего вида:
  - Первая строка — короткое описание по-русски;
  - После описания идет пустая строка;
  - Далее @param и @return (между ними без пустых строк).
Пример:
...
- Бизнес логика должна находиться в классах сервисах модели.
- Любой запрос в БД должен находиться в репозитории модели.
- Методы помощники (вспомогательные) пиши в сервисах модели.
- В ресурсных классах не должно быть логики, только формирование массивов для ответа, используй аттрибуты модели.
- В инструкции проекта для ИИ не удаляй `[AI_MANIFEST.md]` (в это место вставляется инструкция из laravel-helper).
- Старайся не использовать мелкие вспомогательные методы.
- Для простых условий вместо `if` используй тернарный оператор `?:`, например `!true ?: $a = 1` или `$a = null ?: throw`.
- По возможности старайся использовать `Dto` вместо `array` при return данных и при передачи данных между методами классов и слоями приложения.

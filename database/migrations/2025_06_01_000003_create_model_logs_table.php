...
return new class extends Migration {
    ...
    {
        $connection = config($config = LaravelHelperService::getConnection(ConfigEnum::ModelLog))
            ?? throw new Exception("Не указан параметр в конфиге: {$config}");
        $table = config($config = LaravelHelperService::getTable(ConfigEnum::ModelLog))
            ?? throw new Exception("Не указан параметр в конфиге: {$config}");
        ...
        Schema::connection($connection)->create($table, function (Blueprint $table) {
            ...
            $userTableName = lhConfig(ConfigEnum::ModelLog, 'user.table_name');
            $userPrimaryKeyName = lhConfig(ConfigEnum::ModelLog, 'user.primary_key');
            $userPrimaryKeyType = lhConfig(ConfigEnum::ModelLog, 'user.primary_type');

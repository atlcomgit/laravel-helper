...
- При запуске команд в консоли не используй: `2>&1` (приводит к зависанию команды)
...
- Внутри конструкции `match` отделяй `deafult` пустой строкой.
- Внутри метода отделяй `return` пустой строкой.
...
- Не подключай трейт `RefreshDatabase`, т.к. используется транзакция в `DefaultTest`
